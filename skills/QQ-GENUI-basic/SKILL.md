...
执行前必须先读取 `references/failure-log.md`，提取与当前任务相似的失败模式；执行后若发生失败、偏差或需用户反复调整，必须将原因与解决方式系统性回填到该日志中。
...
### Gate 0：读取 Failure Log（必做）

- 先读取 `references/failure-log.md`
- 检索与当前任务最相似的失败记录（页面类型、组件组合、交互、图标、本地化、背景色、状态栏等）
- 提炼出本次任务的"高风险错误清单"，在后续 Gate 中优先规避
- 若日志为空，则继续执行，但后续若出现失败或反复调整，必须在任务结束后补记

通过条件：

- 已读取 `references/failure-log.md`
...
- 若本次任务出现失败、偏差、返工，或用户需要反复指出问题并调整，先回填 `references/failure-log.md`
- 回填内容必须包括：失败表现、根因分析、解决方式、预防规则、关联组件/规范
