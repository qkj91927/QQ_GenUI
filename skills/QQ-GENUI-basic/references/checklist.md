...
> 规则：每个 Gate 未通过不得进入下一 Gate。规范文档和 Token 远程读取，禁止克隆仓库。页面用到的图标必须下载到本地 `assets/icons/`。每次执行前先读 `failure-log.md`，执行后按需回填。

## Gate 0：读取 Failure Log

- [ ] 已读取 `references/failure-log.md`
- [ ] 已检索与当前任务相似的历史失败记录
- [ ] 已提炼本次任务的高风险错误清单
- [ ] 若日志为空，已确认本次任务结束后需按需补记
...
- [ ] 未达100%时已附未修复问题清单
- [ ] 已附审查与完善建议（四维度）
- [ ] 若本次出现失败、偏差、返工或反复调整，已回填 `references/failure-log.md`
