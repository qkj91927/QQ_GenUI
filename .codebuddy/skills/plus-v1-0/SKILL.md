---
name: plus-v1-0
description: 该 skill 用于基于 plus-v1.0 设计系统资源包产出移动端页面与组件方案；当用户要求按仓库规范生成 UI 时强制启用 Gate 流程，逐步验收且不可跳步，并执行最多3轮回归修复，若仍不合规则继续交付并附问题清单。
---
...
使用本 skill 将 `plus-v1.0` 资源包转化为“强约束可执行流程”。
...
- 拉取仓库：`https://github.com/qkj91927/test_component`
- 目标分支：`plus-v1.0`
- 确保本地工作目录基于远程 `plus-v1.0` 最新内容
- 本地已存在仓库时，先同步后继续
...
- 明确页面类型、信息层级、关键交互
- 从 20 个基础母组件中选择候选
- 如用户明确提及 Pro/高级/QUI+，可在候选中加入 `PRO_navbar` / `PRO_tabbar`
...
- 若启用 Pro 组件，补读 `PRO_*_COMPONENT_SPEC.md` 并检查混用规则
- 不合法组合必须替换为合法变体
...
- 禁止在未明确要求 Pro 组件时默认替换基础组件

## 参考资料

- `references/component-index.md`：组件速查与文件映射
